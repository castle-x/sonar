...
ConfigYamlTmplPath=$CURDIR/config/$ConfigVersion/config.yaml.tmpl
...
# hzx wire
cd $CURDIR/cmd/$BinaryName/app
# TODO. not run now
# echo "hzx wire"
if [ $? != 0 ]; then
    print_status "FAILED" "hzx wire" $RED
    exit 1
fi
print_status "SUCCESS" "hzx wire" $GREEN
cd $CURDIR

# Build
print_status "RUNNING" "Building $BinaryName for $OS platform ..." $BLUE
CGO_ENABLED=0 go build -ldflags="-s -w" -o $OUTPUT $CURDIR/cmd/$BinaryName/$BinaryName.go
if [ $? != 0 ]; then
